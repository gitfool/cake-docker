...
# Install Cake tool
RUN dotnet tool install Cake.Tool --version 1.0.0 --tool-path /cake \
...
RUN curl -fsSL https://download.docker.com/linux/static/stable/x86_64/docker-20.10.4.tgz | tar -zxO docker/docker > /usr/local/bin/docker \
    && chmod +x /usr/local/bin/docker \
    && docker --version

# Install docker-compose
RUN curl -fsSL https://github.com/docker/compose/releases/download/1.28.5/docker-compose-$(uname -s)-$(uname -m) -o /usr/local/bin/docker-compose \
